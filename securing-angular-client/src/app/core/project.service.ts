...
export class ProjectService {
    ...
    getProjects(): Observable<Project[]> {
        return from(this._authService.getAccessToken().then(token => { // from() so that it is again converted ot Observable.
            const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`); // Creates the needed header for the API and then we include it in the bottom line.
            return this._httpClient.get<Project[]>(Constants.apiRoot + 'Projects', {headers: headers}).toPromise(); // toPromise so that some code (that obv uses promise) does not have to change.
        }));
    }
