...
export class AuthService {
  ...
  login() {
    return this._userManager.signinRedirect(); // Then after clicking the login button this just uses the usermanager's interface methods
  }
  ...
  completeLogin() {
    return this._userManager.signinRedirectCallback().then(user => {
      this._user = user;
      this._loginChangedSubject.next(!!user && !user.expired); // next() fires loginchangedsubject event
      return user;
    });
  }
